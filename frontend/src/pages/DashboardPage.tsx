...
import type { DatasetDescriptor, ExperimentPreview, Job, ReportBuildRequest } from '../api/types';
...
export function DashboardPage() {
  ...
  const [outputParent, setOutputParent] = useState('/home/hcis-s05/ysws/PISA/pisa-sample-tools/analysis');
  ...
  const buildBrowser = useQuery({ queryKey: ['report-build-browser', buildBrowsePath], queryFn: () => api.datasets.browse(buildBrowsePath || undefined), enabled: reportOpened, retry: 1 });
  ...
  const buildValid = Boolean(outputParent.trim() && reportName.trim() && (sourceKind === 'results' ? parsedResults.length && compatibility.data?.compatible !== false : campaignPath.trim()));
  ...
  function openBuildReport() {
    setOutputParent(libraryPath || browser.data?.path || '/home/hcis-s05/ysws/PISA/pisa-sample-tools/analysis');
  ...
  return (
    <>
      ...
      <Group justify="space-between" mt="xl" mb="md" align="flex-end">
        ...
        <Stack>
          ...
          <Card withBorder p="md">
            <Group align="flex-end" wrap="wrap">
              ...
              <Button variant="light" onClick={() => { const parent = buildBrowsePath || buildBrowser.data?.path || ''; if (parent) setOutputParent(parent); }}>Use as report output</Button>
            </Group>
          ...
          <Card p="md"><Group justify="space-between" mb="sm"><div><Text fw={650}>Experiments to compare</Text><Text size="xs" c="dimmed">Only experiments with identical sampler points and compatible scenario/map provenance can be built together.</Text></div><Group gap="xs"><Button size="compact-sm" variant="default" leftSection={<IconFolderSearch size={15} />} onClick={() => setBuildBrowsePath(DEFAULT_RECORD_PATH)}>Browse records</Button><Badge>{experiments.length}</Badge></Group></Group>{experiments.length ? <Stack gap="xs">{experiments.map((item, index) => <Card key={item.results} withBorder p="sm"><Group justify="space-between" wrap="nowrap"><div><Text size="sm" fw={600}>{index + 1}. {item.dataset_id}</Text><Text size="xs" c="dimmed">{item.simulator ?? 'unknown sim'} · {item.av ?? 'unknown AV'} · {item.sampler ?? 'unknown sampler'} · {item.run_count} runs</Text><Text className="pisa-code" size="xs" c="dimmed">{buildBrowser.data?.path && item.results.startsWith(`${buildBrowser.data.path.replace(/[\\/]+$/, '')}/`) ? item.results.slice(buildBrowser.data.path.replace(/[\\/]+$/, '').length + 1) : item.results}</Text></div><Group gap="xs" wrap="nowrap"><Button variant="subtle" size="compact-xs" leftSection={<IconFolderSearch size={14} />} onClick={() => setBuildBrowsePath(item.results)}>Browse</Button><Button color="red" variant="subtle" size="compact-xs" onClick={() => setExperiments((values) => values.filter((entry) => entry.results !== item.results))}>Remove</Button></Group></Group></Card>)}</Stack> : <Text size="sm" c="dimmed">Browse and preview a record folder, then add it here.</Text>}{compatibility.isLoading && <Text size="xs" c="blue" mt="sm">Checking sampler-point compatibility…</Text>}{compatibility.data && <Alert mt="sm" color={compatibility.data.compatible ? 'teal' : 'red'} title={compatibility.data.compatible ? 'Experiments are directly comparable' : 'Experiments cannot be compared'}>{compatibility.data.compatible ? 'Sampler points, scenario, map, sampler and OpenDRIVE provenance match.' : <Code block>{JSON.stringify(compatibility.data.errors, null, 2)}</Code>}</Alert>}</Card>
          </>}
          <Divider label="Build settings" labelPosition="center" />
          <SimpleGrid cols={{ base: 1, sm: 3 }}><Group align="flex-end" wrap="nowrap" style={{ gridColumn: 'span 2' }}><TextInput label="Output parent" description="Generated reports are written here." value={outputParent} onChange={(event) => setOutputParent(event.currentTarget.value)} style={{ flex: 1 }} /><Button variant="default" onClick={() => setBuildBrowsePath(outputParent)}>Browse</Button></Group><TextInput label="Automatic report name" description="Generated from scenario, components and sampler." value={reportName} onChange={(event) => setReportName(event.currentTarget.value)} required /></SimpleGrid>
          <Alert color="gray" title="Resolved output">{outputDir}</Alert>
          ...
          <Group justify="space-between">
            ...
            <Group><Button variant="default" onClick={reportControls.close}>{buildJob.data?.state === 'succeeded' ? 'Done' : 'Close'}</Button>{buildJob.data?.state === 'succeeded' && buildJob.data.report_id ? <Button onClick={() => { reportControls.close(); navigate(`/reports/${encodeURIComponent(buildJob.data!.report_id!)}/overview`); }}>Open report</Button> : <Button loading={build.isPending} disabled={!buildValid || ['queued', 'running'].includes(buildJob.data?.state ?? '')} onClick={buildReport}>Build report</Button>}</Group>
          </Group>
        </Stack>
      </Modal>
